---
title: Linux 文本查找与处理命令
published: 2026-03-15
description: 整理 cat、less、grep、awk、sed、find 与 rg 的文本查找和处理方法
tags: [Linux, 文本处理]
category: 经验分享
draft: false
---

在 Linux 中查找和处理文本时，`grep` 只负责其中一部分。查看大文件、按列筛选、批量替换或跨目录搜索，还需要结合 `less`、`awk`、`sed`、`find` 和 `rg`。下面按使用场景整理这些命令。
