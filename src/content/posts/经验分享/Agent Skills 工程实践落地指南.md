...
title: Agent Skills 工程实践：把研发约束写进工作流
description: 基于 addyosmani/agent-skills 整理 Skill 选择、研发流程接入、质量门禁和团队落地方法
...
`agent-skills` 把研发约束写成 AI 可以按步骤执行的工作流。
...
这里的 Skill 更接近给 Agent 使用的工程 SOP，而不是面向人阅读的知识库。
...
1. **明确步骤**：Skill 记录具体流程，不承担百科式知识整理。
...
我更倾向于把 `agent-skills` 看成一组可复用的研发约束。它把“什么时候先澄清需求、什么时候必须测试、完成后拿什么证明”写清楚，减少 Agent 每次临场发挥带来的偏差。
...
最终目标很朴素：让 Agent 的改动有明确边界，有验证证据，出现问题时也能回滚。
