---
title: 用 Python 自动创建滴答清单任务
published: 2026-02-25
description: 记录使用滴答清单 Open API 和 Python 自动创建任务，并补充失败重试与飞书告警
tags: [Python, 自动化, 工具]
category: 经验分享
draft: false
---
报错日志、定时提醒和邮件待办分散在不同平台时，可以通过滴答清单（Dida365/TickTick）的 Open API 统一创建任务。

下面用 Python 编写一个滴答清单任务创建工具，支持命令行传参、失败重试和飞书告警。
...
如果在自动化系统里调用该脚本（如定时任务、告警回调），建议保证 `--title` 可追踪，并为 `--priority` 制定明确规则，方便后续排查。
