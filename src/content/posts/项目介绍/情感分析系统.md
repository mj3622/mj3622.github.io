...
description: 介绍基于 Spring Boot 的 B 站评论爬取与情感分析系统，以及 JWT 鉴权、JVM 缓存、数据库分表和异步任务等后端实现
tags: [Spring Boot, 项目实战, 数据分析]
category: 项目介绍
draft: true
---
...
查询视频信息时，可以把结果加入缓存，减少重复访问数据库。这里使用 `Spring Cache` 作为缓存抽象层，并通过它接入 Ehcache、Redis 或 Caffeine 等具体实现。
