...
并发程序需要先把工作拆成任务，再决定由哪些线程执行。这里对比串行处理、为每个任务创建线程和使用线程池三种方式。

**策略一：串行地执行任务**

最简单的策略是在单个线程中串行执行任务。下面的 Web 服务会按顺序处理请求，逻辑容易理解，但一个慢请求会阻塞后续请求，因此不适合需要并发处理的服务。

> [!TIP]
>
> 串行并不总是坏事，大多数 GUI 框架会把界面事件集中到一个线程中处理，以免多个线程同时修改界面状态
...
为每个请求创建新线程可以提高响应性，但线程的创建、销毁和运行都会消耗资源。线程数量过多时，调度开销会拖慢系统；超过可用资源上限后，还可能触发 `OutOfMemoryError`。
...
Java 的 `Timer` 只用一个线程执行所有延迟和周期任务。某个任务运行过久时，其他任务会随之延迟；如果任务抛出未检查异常，定时线程还会直接终止，后续任务也不会继续执行。
