...
description: Redis 进阶学习笔记，涵盖持久化、主从、哨兵、分片和多级缓存
...
缓存把热点数据放在内存中，读取速度快，可以减少数据库访问和压力。按部署位置可分为两类：
...
- 遵循基本格式：`[业务名称]:[数据名]:[id]`，例如`login:user:10`
- 长度不超过44字节
- 不包含特殊字符
...
- 避免key冲突
- 方便管理
- 更节省内存。key是`string`类型，底层编码包含`int`、`embstr`和`raw`三种。`embstr`在小于44字节使用，采用连续内存空间，内存占用更小



#### 2. Big Key

Big Key 指的是键值对中的值非常大的键，例如：
...
- **操作耗时**：访问、修改或删除一个Big Key会耗费较多时间，可能导致 Redis 阻塞其他请求。
- **内存占用**：Big Key可能占用大量内存，影响其他数据的存储。
- **网络传输压力**：如果Big Key被频繁访问，会导致网络传输的开销增加，影响系统性能。
- **排查困难**：在系统性能下降时，定位Big Key可能比较困难，尤其是在数据量大的情况下。



**寻找Big Key的方法：**

- **`redis-cli --bigkeys`**：利用`redis-cli`提供的`--bigkeys`参数，可以遍历分析所有key，并返回Key的整体统计信息与每个数据的Top1的big key
- **`scan`扫描**：自己编程，利用scan扫描Redis中的所有key，利用`strlen`、`hlen`等命令判断key的长度（此处不建议使用`MEMORY USAGE`）
...
![image-20241224224148574](./assets/image-20241224224148574.png)

**存在的问题**：hash的entry数量超过500时，会使用哈希表而不是`ZipList`，内存占用较多

**解决方法：**拆分为小的hash，将id／100作为key，将id%100作为field，这样每100个元素为一个Hash

![image-20241224224327115](./assets/image-20241224224327115.png)
...
例如`MSET`、`HMSET`这类原生批量插入命令。

但是这些只能操作部分数据类型，因此如果有对复杂数据类型的批处理需使用别的方式。



**Pipeline：**

Redis 的管道允许客户端一次性发送多条命令，而无需等待每条命令的响应。
...
![image-20241224230209978](./assets/image-20241224230209978.png)
...
4. 设置合理的rewrite阀值，避免频繁的bgrewrite
...
1. 触发条件：
   - 当命令的执行时间超过指定的阈值（`slowlog-log-slower-than` 配置项），Redis 就会将该命令记录到慢查询日志中。
2. 存储方式：
   - 慢查询日志存储在内存中，默认最多保存 128 条（可通过 `slowlog-max-len` 配置修改）。
   - 慢查询日志不会写入磁盘，因此不会影响性能。
3. 单位：
   - Redis 记录的执行时间以微秒（`µs`）为单位。
...
![image-20241224232339926](./assets/image-20241224232339926.png)
...
![image-20241224233321445](./assets/image-20241224233321445.png)

![image-20241224233851368](./assets/image-20241224233851368.png)
