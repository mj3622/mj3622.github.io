...
description: 通过 Pandas 销售数据分析、SciPy 统计检验和 FastAPI 数值计算服务三个案例，记录相关 API 与统计思路
...
下面通过三个独立案例，记录代码涉及的统计思路和 Python API。

## 第一部分：Pandas 中的描述性统计 (Descriptive Statistics)

处理矩阵数据（DataFrame）时，需要先分清统计操作按行还是按列执行。这里用**股票市场数据**或**多店铺销售数据**举例。
