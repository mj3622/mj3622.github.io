...
description: 介绍 Java 虚拟线程的概念、工作原理、使用方式和适用场景
tags: [Java, 虚拟线程]
category: 编程实践
draft: false
---
...
下面从概念、工作原理、基本用法和适用场景四个方面介绍虚拟线程。
...
* **低开销**：创建成本较低，栈空间可以随使用情况伸缩，单机可创建大量虚拟线程。
...
* **效果**：单机可以承载大量并发连接；等待数据库响应时，虚拟线程会被挂起，不会持续占用平台线程。
...
虚拟线程适合大量任务同时等待网络、数据库或文件 I/O 的场景，可以继续使用同步代码结构，同时降低平台线程长期阻塞的成本。

* **最适合**：Spring Boot Web 服务、微服务调用聚合、爬虫、大量数据库操作。
* **不适合**：CPU 密集型计算。

是否迁移仍要通过压测确认，并检查线程固定、连接池容量、`ThreadLocal` 使用和监控工具的兼容性。
